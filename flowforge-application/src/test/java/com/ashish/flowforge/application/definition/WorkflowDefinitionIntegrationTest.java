...
class WorkflowDefinitionIntegrationTest {
    ...
    void duplicateWorkflowVersionIsRejected() {
        ...
        DefinitionException ex = assertThrows(DefinitionException.class, () -> service.createDraft(workflow("duplicate-test", 1)));
    ...
    void draftCanBeUpdated() {
        ...
        var updated = new WorkflowDefinitionInput("update-test", 1, "Updated", "changed", Map.of("type", "object"), List.of(task("first", List.of()), task("second", List.of("first"))));
    ...
    void tenConcurrentRequestsCreateOnlyOneVersion() throws Exception {
        ...
        try {
            List<Future<?>> futures = java.util.stream.IntStream.range(0, 10).mapToObj(i -> pool.submit(() -> {
                start.await();
                try {
        ...
        assertEquals(1, jdbc.queryForObject("SELECT count(*) FROM workflow_definition WHERE workflow_key=? AND version=1", Integer.class, key));
    }

    private WorkflowDefinitionInput workflow(String key, int version) {
        return new WorkflowDefinitionInput(key, version, "Document Approval", "Reviews a submitted document", Map.of("type", "object"), List.of(
    ...
    private TaskDefinitionInput task(String key, List<String> dependencies) {
        return new TaskDefinitionInput(key, key, "WORKER", List.of("TEST"), 60,
                new RetryPolicyInput(3, 5, 2.0, 60, true), Map.of(), dependencies);
    }
}
